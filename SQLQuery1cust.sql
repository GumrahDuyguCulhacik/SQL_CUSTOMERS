...
SELECT TOP 20 NAMESURNAME, AGEGROUP FROM CUSTOMERS
...
SELECT C.CITY,  D.DISTRICT, 
	COUNT(CO.ID) AS CUSTOMERCOUNT 
FROM CUSTOMERS CO
INNER JOIN CITIES C ON C.ID = CO.CITYID
INNER JOIN DISTRICTS D ON D.ID = CO.DISTRICTID
GROUP BY C.CITY,D.DISTRICT
ORDER BY C.CITY, COUNT(CO.ID) DESC


WITH DistrictCounts AS  --WITH-- Eine temporäre Tabelle mit Namen "DistrictCounts" wird erstellt
(SELECT
C.CITY,
D.DISTRICT,
COUNT(CO.ID) AS CUSTOMERCOUNT,
ROW_NUMBER() OVER 
	(PARTITION BY C.CITY 
	ORDER BY COUNT(CO.ID) DESC) AS RN --PARTITION BY- C.CITY--Das Datenset wird nach Städten in Partitionen aufgeteilt
FROM CUSTOMERS CO
INNER JOIN CITIES C ON C.ID = CO.CITYID
INNER JOIN DISTRICTS D ON D.ID = CO.DISTRICTID
GROUP BY C.CITY, D.DISTRICT)
